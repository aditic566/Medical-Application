...
export class HttpService {
  ...
  //   return this.http.get<any>(`${this.serverName}/api/user/check-email/${email}`, {
  //     headers: new HttpHeaders({
  //       'Content-Type': 'application/json'
  //     })
  //   });
  ...
  //   return this.http.put<any>(`${this.serverName}/api/user/check-email/${orderId}?newStatus=${newStatus}`, {}, {
  ...
  updateEquipment(equipmentId: number, data: any): Observable<any> {
    return this.http.put<any>(this.serverName + `/api/hospital/equipment/update/${equipmentId}`, data, {
      headers: this.getHeaders()
    });
  }
  ...
  deleteHospital(hospitalId: number): Observable<any> {
    return this.http.delete<any>(this.serverName + `/api/hospital/${hospitalId}`, {
      headers: this.getHeaders()
    });
  }

  deleteEquipment(equipmentId: number): Observable<any> {
    return this.http.delete<any>(this.serverName + `/api/hospital/equipment/${equipmentId}`, {
      headers: this.getHeaders()
    });
  }
}
