...
export class MaintenanceComponent implements OnInit {
  ...
  getMaintenance(): void {
    this.httpService.getMaintenance().subscribe({
      next: (data) => {
        ...
        console.log(this.maintenanceList); // Log the data to inspect it
      },
      error: (error) => {
        this.showError = true;
  ...
  updateCompletionDate(maintenance: any, event: Event): void {
    ...
    if (newDate) {
      ...
      console.log(`Updating completion date for maintenance ID ${maintenance.id} to ${newDate}`);
  ...
  updateStatus(maintenance: any, event: Event): void {
    ...
    if (newStatus) {
      ...
      console.log(`Updating status for maintenance ID ${maintenance.id} to ${newStatus}`);
      this.updateMaintenance(maintenance);
    }
  }
  ...
  updateMaintenance(maintenance: any): void {
    this.httpService.updateMaintenance( maintenance,maintenance.id).subscribe({
      next: () => {
        ...
        console.log(`Maintenance record ID ${maintenance.id} updated successfully`);
        this.getMaintenance();
